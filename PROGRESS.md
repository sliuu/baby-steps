...
**Now:** A vertical hairline separates the calendar from the sticker tray on desktop, and the rail grew 2rem to pay for it. Before that: the app is printed on newsprint instead of paper. Nothing is `#ffffff` any more — the page is a warm-neutral grey and a card *lifts off* it rather than going white — and today is marked by a 2px press-red rule under its numeral, the only red in the app and the only thing this colour is ever used for. The retheme also fixed an accessibility failure that predated it: the muted ink was 3.62:1 on the old page, below the app's own 4.5 floor, and is 4.85:1 now. Checked on Today, Week, Month, Trends, the day sheet and the tray at 1440 and 390, light and dark. **Dark is proposed, not designed — derived to hold the light theme's relationships, measured for contrast, never seen beside a frame.** Before that: the app is set in two typefaces instead of three. **Schibsted Grotesk** is the whole interface — titles and body and labels, one family — and **Spectral Light Italic** is the first italic the app has ever loaded, reserved for the four places where somebody is *writing* rather than the app reporting: a day's note, the word for how a day felt, "Quiet" on an empty day, and "about every 3 days" under a habit. Syne, DM Sans and Space Mono are gone. Schibsted turned out to run 5–11% *larger* than Syne rather than smaller, so the heading rungs carry a measured 0.94 correction, and two sizes outside those tokens moved with it — the phone's week title, which had started truncating, and the phone bottom bar's padding, where "Trends" was losing its last two letters to a quarter of a pixel. Checked on every screen at 1440 and 390, light and dark. Before that: every control in the app turns the same 4.8px corner — the one the week view cuts its sticker bars with. The nav segments, the Areas · Habits · Moods pill, both Trends dropdowns and the demo's sign-in link were stadiums and aren't any more; circles that are circles by design stayed round. The desktop calendar also stopped scrolling past its own content: the sticker rail's hidden overflow was buying the page ~150px of blank space below the fold, and a further one-pixel error in its viewport cap was raising a scrollbar for a single pixel. Today, Week and Month all end exactly at the fold on a full-screen MacBook now. Before that: Trends looks like the design handoff. Areas · Habits · Moods is a tracked pill on every screen width, and the phone's bottom nav is the same pill. Areas is ranked colour bars (beside the star on a wide screen), Habits is a sticker list on a phone and the table alone on a wide screen — no ranking above it, and no range picker of its own any more — and Moods is mood bars and the day-by-day line, stacked on a phone and side by side on a wide screen. Seen in headless Chrome at 390 and 1440. Before that: the app has a mark. Three stickers in a triangle — red, blue and green, 45% tints with a white die-cut rim — sit in the browser tab and on the iOS home screen, and `create-next-app`'s Vercel triangle and its five sample SVGs are gone from the repo. Before that: Trends' Habits tab is a ranking over a table, and the eight-week strip that used to sit between them is gone — its drawing moved into the table's "When" column, where the ticks became blocks that follow the range: a block a day up to a month, a week up to a year, a month past that. The table has no title, its dates and its **own** range dropdown in the top left (independent of the one governing the rest of the page), an area filter on the right, four sortable headlines, and a pager bottom right saying which page of how many. Every panel on Trends can be read over any of eight ranges — This week, Last 7 days, This month, Last 30 days, This year, Last 12 months, All time, Custom. Before that: a second pass over the same surface, all of it chrome and none of it structural: a tray row is now drawn in whichever shape the calendar beside it is drawing — the month's circle with the name next to it, or the week's named rectangle with the tint filling the whole row — the rail's heading sits still while the stickers under it scroll down a scrollbar drawn to match the app (a rem wider to pay for the width that bar takes), the month's cells went from white to the page's cream, and then nothing on the page is a card any more: the week, the month and all five Trends panels are one thick rule with their content hanging under it, no fill, no radius, no sides and no bottom, with thin lines only inside. The two empty states are the same rule, dashed. Day names are in Space Mono with everything else that labels something. Still never seen in a browser — the extension has been unavailable for five sessions, and the week view can't even be curled, because Month/Week is a hash route. Before that: six passes over the interface, none of them structural: the eyebrows moved to Space Mono, every sticker gained a white die-cut rim and a soft shadow, the five moods lost their words, the tray narrowed from 18rem to 13rem, and the page's cap moved from 72rem to 96rem so the body stretches to 1536px before it stops. The seventh was asked for and declined on the face's own terms — Instrument Serif ships one weight, so there is no bold to add. Before that: There is a demo at **`/demo`**, linked from under the sign-in buttons: the whole app, no account, a year of stickers generated from today by `lib/demo.ts` — four a day, spread across the six areas, 86% of days with a mood, and a habit that visibly stopped. The boards are the real ones; `local` on `CalendarBoard` turns off the network and nothing else. Before that: Trends was re-cut and then split into three tabs — **Areas · Habits · Moods**. No pie, no chart switcher. *Moods* opens with a line joining every day you logged, in ink, with the reading printed above it in words. Never yet seen in a browser. Before that: the calendar grew two views. A Month/Week pill sits beside the title and the same two arrows step whichever one you're in; the week is seven tall columns, each with the day's stickers spelled out as named bars and a note field at the bottom for a line about that day. Before that: every emoji in the app became a Lucide icon, drawn from a 432-icon map with eight hand-drawn sports additions, and the body text moved to DM Sans throughout. **Committed and pushed through this step.** `npm run db:notes` and `npm run adopt:icons` have both been run, so existing stickers carry Lucide icons rather than emoji. The `lib/database.types.ts` item below is still open. One step left after this: deploy.
...
## 2026-09-24 · A line between the calendar and the tray

"Add a vertical hairline between the calendar and the sticker tray, as in frame 1b … Put it on the tray's container as a left border … Give the tray `pl-8` … Hide it at the breakpoint where the tray stacks."

**What changed about the interface.**

- **The tray is a column now, not a margin.** A 1px `--hairline` rule runs down its left edge for the full height of the rail, from the top of the content area to wherever the rail's cap ends. The calendar and the tray were previously separated by 40px of nothing, which read as "the page happens to stop here"; the line makes the rail a region with an edge. It shows in Today, Week and Month — the brief named two, but the rail is one element in one place for all three, so leaving Today out would have meant special-casing it.
- **The rail is 16rem instead of 14.** Directly the line's doing — see below.
- **Nothing else moved.** No divider element, no shadow, no background on the tray, no change to the calendar grid's own borders. The 40px gap between the two columns is the gap it was.

**Decisions**

- **`pl-8` on a fixed-width rail takes its 32px out of the *inside*, and the rail's width was measured rather than picked.** The documented floor is 13rem of content plus a rem for the list's own scrollbar. Left at `lg:w-56`, the border and its padding cut a sticker name's slot from **136px to 103px** — 18 characters down to **13**, against a `NAME_MAX` of 24, so a third of the names a person can type would have started truncating. `lg:w-64` hands back exactly the 33px the line costs and the slot lands at 135px, 18 characters again. Measured on a hidden span in the row's own font, before and after. **This is the one thing not in the brief;** the alternative was shipping a five-character cut to every sticker name to save 32px of calendar. The calendar is `flex-1`, so seven columns share the loss at about 4.5px each.
- **A left border on the `<aside>` rather than a divider element, as asked, and it pays for itself twice.** The rule inherits the rail's stickiness and its `max-h` cap for free, so it starts where the rail starts and ends where the rail ends with nothing to keep in sync. It also inherits the rail's `hidden lg:flex` — so the "hide it when the tray stacks" requirement needed no code at all. Verified at 390, 900, 1023, 1024 and 1440: below `lg` the computed border width is 0px because the element is `display: none`, and at 1024 it is 1px with 32px of padding.
- **The rule starts 40px below the nav's bottom border, not flush against it.** That 40px is `AppShell`'s `py-10`, the same padding the calendar column starts at, so the line begins level with the page title rather than level with the nav. It satisfies "don't cross the nav's border" and reads as the content area's own edge.

**Changed**

- `components/dnd/CalendarBoard.tsx`: the rail's `<aside>` gained `lg:border-l lg:border-hairline lg:pl-8` and went `lg:w-56` → `lg:w-64`. The width comment above it was re-derived.

**Checked:** `tsc` clean, lint 0 errors, `npm test` 399 passing. Captured at 1440×900 in Month, Week and Today, light and dark, and the rail's computed border measured across the `lg` breakpoint at five widths.

**Open**

- **Still no frame-1b comparison** — Paper Desktop isn't running, so the line's position, weight and the rail width are faithful to the brief's words and unverified against the drawing. The rail width in particular is a number I chose.
- **In Week the rule outruns the calendar.** The week's columns end well above the fold and the tray keeps going, so the line continues past the bottom of the panel beside empty space. That is the honest consequence of the line belonging to the tray; capping it at the panel's height would need the separate element the brief ruled out.

---

## 2026-09-24 · Evening Paper, and one red mark

"Retheme the app to the 'Evening Paper' palette. Change values only. The only new names are `--today-mark` and its `@theme` bridge." · "Done when: the four screens plus DaySheet and the tray are checked in light and dark … Check contrast on `--ink-muted` and `--ink-label` over `--bg` and `--surface`."

**What changed about the interface.**

- **The page is newsprint, and there is no white left in it.** `--bg` went from `#fbfbfa` to `#ebeae4` and `--surface` from pure `#ffffff` to `#f3f2ed`. The consequence is a change of direction, not just of value: a card used to be *white on almost-white*, which is a difference you take on faith, and now it is a lighter grey that visibly lifts off a grey ground. The month grid, the tray card, the sheet and every Trends panel gained an edge without anyone drawing a border.
- **Today is a red rule under its numeral, and that is the app's only red.** It replaces the ink ring in the month cell, the week column, the Today header strip and the phone's week list. A ring said "this is a control you can press"; an underline is the annotation a person makes on a printed calendar. `--today-mark` is `#d4271c` in light and `#ff5a47` in dark, and it is deliberately not `--accent` (shadcn spends that on hover and selected surfaces), not `--destructive`, and not for links, errors or buttons.
- **Selection now reads as a press into the page rather than a light shone on it.** `--secondary` / `--muted` / `--accent` moved from `#f0f0ee` to `#e0dfd8`. On the old near-white page that value was a step *down* into grey; on `#ebeae4` the same eleven-point step still goes down, which is why the value moved with the ground rather than staying put. A selected day, a hovered cell and an unfilled sticker chip all still sit below the page.
- **Stickers were not touched and changed anyway.** The six `--ramp-*` hues are untouched, as asked, but every sticker fill is a `color-mix` against `--surface`, so all thirty-six tints re-mixed themselves onto the new ground. They read slightly warmer and less bright, which is the newsprint doing what newsprint does.
- **The muted ink is legible for the first time.** See below.

**Decisions**

- **`--ink-muted` was failing WCAG before this and nobody had measured it.** `#8a8378` on `#fbfbfa` is **3.62:1** — under the app's own stated 4.5 floor, on every eyebrow, every secondary line and the "Quiet" on an empty day. The brief pre-empted the obvious replacement (`#6f6e68`, 4.2:1) and specified `#66655f`, which lands at **4.85 / 5.22 / 4.50** over `--bg` / `--surface` / `--surface-sunken`. The sunken rung is 4.50 exactly, so it is the one to re-check first if any of these move. `--ink-label` `#434341` is 8.23 / 8.85 / 7.63.
- **`rounded-full` had to come off the four numerals, and it is not cosmetic.** A `border-bottom` under a 9999px radius renders as a curved arc, not a rule. Dropping it is invisible on its own — the numerals never had a visible round background — but without it the mark is a smile rather than an underline.
- **Every day carries a border, not just today.** `border-b-2 border-transparent` on the other days, so the numeral's vertical centring is identical whether or not the mark is there. `box-sizing: border-box` means the `size-7` box is unchanged either way.
- **Today also stayed `font-medium`.** The red is 4.24:1 on the page — comfortably over the 3:1 a non-text rule needs, under the 4.5 text would — so the day is never marked by colour alone.
- **The focus ring is still under the bar, and it was before.** `--ring` went `#a8a8a3` → `#a6a49b`, which is 2.31:1 → **2.07:1** on the page. WCAG 2.2 wants 3:1 for a focus indicator. This is a carry-over of an existing failure, not a new one, and it is the one number in the brief worth arguing with. Dark improved slightly, 2.21 → 2.55, and is also short.
- **The sticker shadow was neutralised to match the ink.** It is a hand-written literal rather than a mix — a shadow must not flip with the theme — and it was `rgb(28 26 23)`, the old warm ink, on a page that no longer has any warmth. Now `rgb(20 20 20)`, which is the new `--ink` written out. Same alphas. Not in the brief; the comment claiming it was "the light theme's ink" had otherwise become false.
- **Dark is proposed, not designed, and the file says so at the top of the block.** The values hold the light theme's relationships on a dark ground and every one clears 4.5 with room, but none has been seen beside a frame. Two places it does not mirror the light theme, both noted in the comments: the selected surface lifts rather than sinks, because there is nowhere below `#161615` to go; and the mark brightens to `#ff5a47`, because `#d4271c` on near-black is 2.2:1 — a mark you have to hunt for.

**Changed**

- `app/globals.css`: the `:root` and `.dark` grounds, ink rungs, hairline, rule, scroll thumb and the shadcn slots; new `--today-mark` in both themes; `--color-today-mark` in `@theme inline`; the comment blocks rewritten to describe newsprint grey and one press-red mark, and every stale hex in the surrounding prose corrected.
- Four components lost the ink ring and gained the rule: `DayCell`, `WeekDayColumn`, `TodayView`, `WeekList`. `rounded-full` came off the numeral in all four.

**Checked:** `tsc` clean, lint 0 errors, `npm test` 399 passing. Captured over CDP in headless Chrome at 1440×900 and 390×844, light and dark: Today, Week, Month, Trends, the phone month and week, the day sheet and the tray.

**Open**

- **The frame-1b comparison was not run.** Paper Desktop is not running, so "Directions Desktop.dc.html" could not be opened; the file is not in the repo or in Documents/Downloads/Desktop either. Every value in the brief was applied literally and verified numerically, but nothing has been held up against the design.
- **`--ring` at 2.07:1.** Under the 3:1 focus-indicator bar, as its predecessor was.
- **The phone's month grid has no today mark.** `MonthDots` never had the ink ring, so the brief's "it replaces the current ink ring" left it out; today there is a grey box and a medium weight. That means `bg-secondary` says "today" in one view and "the day you're reading" in another. Worth a decision, not a silent fix.

---

## 2026-09-24 · Two faces, and the second one is only ever italic

"Swap the app's typefaces. Change values only — don't rename existing tokens or utilities." · "Schibsted Grotesk … Spectral … Done when: no component files changed except those gaining `note`, and every screen has been checked in light and dark."

**What changed about the interface.**

- **One text face instead of three.** Schibsted Grotesk sets the titles, the body, the eyebrows and the weekday row. A heading is now distinguished from a paragraph by size alone — Syne's wide display drawing is gone, and so is Space Mono, which had been carrying eight words a screen.
- **The app has an italic, and it says exactly one thing.** Spectral Light Italic wears the `note` utility, and `note` is on four things: the note under a day (week column, day sheet and Today), the word for how a day felt (the five faces in Today, the sheet and the mood popover), "Quiet" on a day with nothing on it, and the cadence line in Trends — "About every 3 days". Every one of those is somebody writing or a reading offered softly. Nothing the app *counted* is italic: marks, dates, percentages, habit names, area names and "Last" all stay upright.
- **The labels came back into the family.** The eyebrows and the weekday row are Schibsted at small sizes now. What makes them read as labels is the uppercasing, the tracking and the label ink rather than the face. Both went up a size on the way — a proportional face at a mono's nominal size reads a rung smaller, because a mono spends its advance on air.
- **The headings are about 6% smaller than the numbers they were on.** Not a design change; a correction, so they land where Syne's did.

**Decisions**

- **The brief's premise was wrong, and it was worth measuring rather than trusting.** It expected Schibsted to run *smaller* than Syne. Set at a 100px em and measured on a canvas, it runs larger on every axis: x-height 52.7 vs 50.0, cap 70.3 vs 65.0, and the same string 11% wider. Left uncorrected that produced a real regression — the phone's week title "20 – 26 Sept 2026" came to 251.6px inside a 252px box, which fit on a one-to-one screen and truncated on a retina one. **0.94** is the factor, sitting between the x-height ratio (0.949) and the cap ratio (0.925), and it is applied to all six heading rungs plus `PAGE_TITLE`'s phone size. The title now has 15px of slack.
- **The body scale survived at 0.92.** Schibsted and DM Sans have near-identical caps (70.3 vs 70.0); Schibsted's x-height is 4.6% larger on a string 3.4% narrower, which cancels at reading size. Tried before changing, as asked.
- **`-0.03em` on headings stayed, for a new reason.** It was written to pull a face drawn wide back to a set word. Schibsted arrives already set, but a neutral grotesque at 45–67px still wants tightening, because default tracking is drawn for a paragraph. Same number, ordinary optical-size job now. The comment says so.
- **One word was truncating in the phone's bottom bar.** The active segment is semibold, and Schibsted's semibold "Trends" wants 44.73px where the flex row gave it 44.50 — a 0.23px shortfall, which `truncate` answers with a full ellipsis ("Tren…"). Fixed by trimming `pill()` from `px-2` to `px-1.5`: the segments are equal-width and centred, so nothing visibly moves, and it buys 3.8px rather than another quarter-pixel. Verified before and after by stashing the swap and re-measuring — DM Sans wanted 44.22 and fit, so this was the swap's doing.
- **`--font-mono` was removed.** Nothing but `eyebrow` ever pointed at it, and no code blocks or tabular figures wear it.

**Changed**

- `app/layout.tsx`: two `next/font/google` families in place of three. Spectral is not variable in the cut wanted, so `weight` and `style` are required arrays; Schibsted is variable and takes no `weight`.
- `app/globals.css`: `--font-sans` and `--font-heading` both point at Schibsted; new `--font-note`; `--font-mono` gone; six heading rungs × 0.94; `eyebrow` and `daylabel` re-tuned off the mono; new `note` utility.
- `lib/layout.ts`: `PAGE_TITLE` phone size 2rem → 1.88rem; `pill()` padding `px-2` → `px-1.5`.
- Six components gained `note` and nothing else: `DayNote`, `WeekList`, `MoodRow`, `DayMoodButton`, `HabitList`, `HabitTable`.

**Checked:** `tsc` clean, lint 0 errors, `npm test` 399 passing. Captured over CDP in headless Chrome at 1440×900 and 390×844, light and dark: Today, Week, Month, Trends (Areas · Habits · Moods), the day sheet, the day modal and the signed-out landing page. The landing page's tagline still breaks to three lines at both widths.

**Open**

- **`MonthDots` sets the same "about every N days" sentence as Trends and stays upright**, because the brief scoped `note` to Trends' cadence lines. It is the one place the rule is inconsistent.
- **`DayModal`'s mood list stays upright too.** Its five mood names go through `TrayRowFace`, which is shared with sticker names, so italicising them would italicise "Meditation" as well. It would need a prop, which is a component change the brief ruled out.
- Three comments in component files still name the old faces — `PeriodHeader.tsx:82` ("kept for Syne"), `CalendarSkeleton.tsx:168` ("Space Mono at about 10px") and `CalendarBoard.tsx:683` ("19 characters of DM Sans"). Left alone because the brief said no component file changes beyond `note`; the arguments they make still hold, only the face names are stale. `app/login/page.tsx:82` has the same problem with a live number — its "53.4em of advance in DM Sans" is now about 51.6em, and the three-line conclusion is unaffected.

---

## 2026-09-23 · One corner, and a page that stops where it ends

"Let's stick to the small border radius used on selecting days in the week in the week view, and apply it to things like the pills on nav bar and the pill switcher … in the trends section, or the filter in the trends section" · "in most cases, the scroll for the page is way longer than it needs to be (theres a lot at the bottom thats empty) … For some views (like week) on a standard macbook screen, the page shouldn't actually need to scroll at all"

**What changed about the interface.**

- **Every control turns the same corner, and it is the week's.** The top nav's segments, Trends' Areas · Habits · Moods pill, the range dropdown, the custom-date button, the Habits area filter and the demo's "Sign in" were all stadium-shaped. They are now `rounded-sm` — 4.8px, the corner the week view already cuts its named sticker bars with. Round survives where round is the drawing rather than the chrome: sticker discs, day numerals, area dots, the avatar, the theme toggle, and the bar charts' ends.
- **The desktop calendar no longer scrolls past its own content.** There was ~150px of blank page under every calendar view on a wide screen. It is gone.
- **On a phone, Week and Trends fit the screen exactly** — 844px of content in an 844px window, no scroll at all.
- **And then the last pixel.** Week and Today still raised a scrollbar for 1px on a full-screen laptop, which reads as a bug in a way a long scroll doesn't. The tray's viewport cap was `100vh - 9rem`: 64px of nav + 40 + 40 of page padding, forgetting the nav's own hairline. It is `100vh - 145px` now. On a 1512×982 MacBook, **Today, Week and Month all end exactly at the fold** — no scrollbar on any of the three.

**Decisions**

- **The radius lives in one constant.** `CONTROL_RADIUS` in `lib/layout.ts` feeds `segment()`, `PILL_TRACK` and `pill()`; the four one-off triggers spell `rounded-sm` at their call site with a comment pointing back at it. One place to retune if 4.8px turns out to be too tight.
- **"Pill" stayed in the names.** `PILL_TRACK` and `pill()` describe what the control *is* — a few choices, all on screen — not the shape it's cut to.
- **The blank scroll was the sticker tray's, and the fix is one class.** The rail scrolls internally and is capped at the viewport, but Chrome still counted the overflow it hides toward the document's scroll height, so a tray with more stickers than fit bought the *page* a few hundred pixels below the fold. `lg:overflow-hidden` on the `<aside>` says that what the rail hides is hidden from the page too. Nothing is clipped that wasn't already, and the drag preview is unaffected — `DragOverlay` renders at the board.
- **What's left is content, not air.** Measured with the demo banner discounted, the calendar views fit outright at 982px tall; at 900 the month is 13px over, and at 845 today is 42 over and the month 68. Today's overflow is its sticker list; the month's is four of six rows running past the 96px cell floor because they hold marks. Shrinking either means shrinking a mark or a gap, which is a design decision rather than a cleanup, so nothing was changed.
- **The phone's 104px bottom padding stayed.** The fixed nav covers 63px of it; the remaining 41px is the gap between the last row and the bar.

**Changed**

- `lib/layout.ts`: new private `CONTROL_RADIUS`, used by `segment`, `PILL_TRACK` and `pill`.
- `components/trends/RangePicker.tsx`, `components/trends/HabitTable.tsx`, `app/demo/page.tsx`: the four remaining stadium controls.
- `components/dnd/CalendarBoard.tsx`: `lg:overflow-hidden` on the tray rail.

**Checked:** `tsc` clean, lint 0 errors, `npm test` 399 passing. Heights re-measured over CDP in headless Chrome at 1512×982, 1512×900, 1512×845 and 390×844 for all four views.

**Open**

- The demo banner pushes the week 42px past the fold on a 845px-tall window. It scrolls away and the nav is sticky, so it was left alone.
- Today and Month still scroll by a screen's worth of genuine content; trimming them means touching the mark size or the month's cell floor.

---

## 2026-09-19 · Trends from the handoff · One pill, three lenses

"let's update the mobile trends page to look like this. Let's keep that pill thing for desktop too … I'd also like to update the bottom bar on mobile/small screens to be in the same style as this pill bar" · "Let's have the bar chart in the areas section become the right hand side of the areas section on desktop too … remove the bar chart on desktop from Habits, so it's just the table … on desktop they'll be side by side (bar chart on the left)"

**What changed about the interface.**

- **The tabs are a pill.** Areas · Habits · Moods sit in a tracked pill: the track is a 6% ink wash, and the active segment is a raised white chip. The pill is full width on a phone and 22rem on a wide screen, with the range picker and its dates on the same line to the right. The range picker is now a pill as well.
- **The phone's bottom nav is the same pill.** Four segments, icon plus word, with the active one raised. The desktop top nav is unchanged.
- **Areas** is six ranked bars. Each has a coloured dot, the name, a count, a share and a 7px track filled relative to the leader, with a "Total · N marks" line under them. On a wide screen the bars are the right-hand column beside the star and replace the percentages-only table. On a phone the star isn't drawn.
- **Habits.**
  - On a phone it is a list: sticker, name, "About every N days", then the count over "Yesterday" / "3 days ago". It is sorted by count, then name. Archived habits fold under an "Archived" row.
  - On a wide screen it is the sortable table alone, with no most-done bars above it. The table has lost its own range picker and now follows the page's range.
- **Moods** is five grey bars, one per face, each with a count, then the day-by-day line. The line is redrawn: faces on the axis at great/okay/rough, a dashed okay gridline, open dots, and first/middle/last dates underneath. The bars and the line are stacked on a phone and side by side on a wide screen, bars on the left. Both columns sit under the page rule. A "Days recorded · N of M" footer runs under both.

**Decisions**

- **The app's fonts and tokens were kept, not the handoff's.** It specified Hanken/Martian/Newsreader. The layout, spacing and pill treatment came across, but the type is still DM Sans, Space Mono and Syne.
- **One range for the whole page.** The Habits table had its own picker. With the ranking above it gone, a second picker governing one panel was the only thing left disagreeing with the rest of the page.
- **Rows are counted once.** The phone list and the wide table draw the same `habitTable` rows, computed in `TrendsBoard`.
- **The mood line keeps the seven-day gap rule.** The handoff draws unmarked days as dashes. This line still breaks across a week unlogged and draws nothing for single missed days.
- **The axis faces are 18px, with a 30px gutter** rather than the handoff's 22px gutter, so they are legible.
- **Unchanged from the handoff:**
  - The range picker is still a dropdown rather than a bottom sheet.
  - The range is not remembered across sessions.
  - `frequencyLabel`'s copy is unchanged and not italic.
  - StickerMark keeps its rim.

**Changed**

- New: `components/trends/AreaBars.tsx`, `HabitList.tsx`, `MoodBars.tsx`.
- Deleted: `AreaTable.tsx`, `MostDone.tsx`.
- `MoodStrip` is gone from `Readout.tsx`.
- Rewritten: `TrendsBoard.tsx` (Radix Tabs primitive, pill), `MoodLine.tsx` and `TrendsSkeleton.tsx`.
- `HabitTable.tsx` takes its rows and window as props.
- `lib/layout.ts` gains `PILL_TRACK` and `pill()`, which are shared by the Trends tabs and `BottomNav`. `AppShell`'s bottom padding grew for the taller nav.
- `design_handoff_trends/` is out of the repo.

**Checked:** `tsc` is clean, lint has 0 errors, and `npm test` passes 391 tests. Screenshots of all three tabs were taken in headless Chrome at 390×844 and 1440×900, because the browser extension was still unavailable.

**Open**

- Dark mode wasn't screenshotted.
- The archived toggle is untested against real data: the demo has no archived habits.

---

## 2026-09-13 · A mark of its own · Three stickers in the tab

"Let's add a couple options that are the two circles, but not overlapping and different color combinations taken from the color palette" · "How about three stickers not overlapping with a true 1.5px rim?" · "Let's try that one in the website, but keep the artifact, we may try new things" · "let's try s-two?" · "Lets try t-tri-tight" · "Tiny bit tigheter"

**What changed about the interface.** The tab is no longer Next.js's logo. It is three stickers in a point-up triangle, drawn the way a sticker is drawn everywhere else in the app: a 45% tint of a ramp hue, a hard white rim, a soft shadow under it. Red at the apex, blue and green on the base, pulled close enough that the three rims fuse into one white backing sheet with channels cut through it. It carries its own dark rules, so on a dark tab strip the discs go to the 40% tints over `#26231f` and the rim goes to the app's `--sticker-edge` grey. The same triangle is the iOS home-screen tile at 180. This is the first drawn symbol the app has ever had — the wordmark in `TopNav` and `DemoShell` is still the words alone, and putting the mark beside them is a separate decision, deliberately not taken here.

Three fixes rode along, all found while looking at the tab. The sign-in button no longer comes back from the browser's back-forward cache still depressed and disabled. The hydration warning that appeared on every load in a browser with ColorZilla installed is gone. And `/apple-icon` is no longer auth-gated.

**Decisions**

- **A rim of 3 units, not 1.5, and that is the whole design.** The app's sticker has a 1.5px rim on a 26px disc. Translated as a *ratio* into a 32-unit viewBox that is 0.74 units, which at 16px is three-eighths of a pixel — the browser draws it as a faint lightening and the three discs collapse into one pale smudge. What matters in a favicon is rendered pixels, so the rim is 1.5 *rendered* pixels: 3 units of a 32 box drawn at 16. Both readings were drawn and compared at 16px before choosing; the ratio version (`t-tri-thin` on the sheet) is the honest translation and the unreadable one.
- **The rims cross; the fills do not.** Tangent discs at a 3-unit rim have to shrink to a fill radius of 3.8 to fit three in the box, and the mark goes pale. At 13.4 between centres with radius 6.7, the rims overlap by 3 units and the fill radius is 5.2 — well over a third more colour — while a front disc's outer edge lands exactly where its neighbour's colour ends. **The radius is half the spacing and that is the rule.** An earlier attempt grew the discs to 6.9 at the same spacing: a bite only 0.4 units deep, invisible at 16px, but a shallow bite is a *wide* one and the flat spanned a third of each disc at 32px. Radius and spacing are one number, not two.
- **Green, not yellow.** Yellow at 45% over white is `#f0dea7`, four percent off the page colour, and it was the disc that went missing on a light tab strip.
- **Centred on the bounding box, not the centroid.** A point-up triangle of circles hangs lower than it sits; centring the centroid puts the apex rim through the top edge of the viewBox.
- **One `feDropShadow`, not the app's two layers.** At this size the second layer draws the same grey over the first and the mark goes muddy rather than lifted. The two opacities do not add — 0.16 and 0.09 composite to about 0.24, which is the single number now used.
- **`app/favicon.ico` had to be deleted, not left alone.** Next emits a `<link>` for both files and several browsers prefer the `.ico`, so leaving it means shipping the new mark and still seeing the Vercel triangle.
- **`public/` went with it.** All five files were `create-next-app` samples with zero references anywhere in `.ts`, `.tsx`, `.css`, `.md` or `.json`. The directory is gone rather than emptied.
- **The iOS tile is generated, not exported.** There is no SVG rasteriser in this toolchain, so a hand-exported PNG would drift from `icon.svg` the first time the mark is retuned. `app/apple-icon.tsx` renders it through `next/og` at build time from coordinates put through one scale factor. It sits on cream rather than transparency, because iOS turns an alpha cut-out into a black tile, and it carries only the light treatment, because a home-screen tile is one image for both appearances.
- **Satori has no filters, so the tile's shadow is per disc.** Where the rims cross that shows as a faint seam and the white cloverleaf becomes three white circles laid over one another. At 180px it reads as stacked paper. It is a consequence of the renderer, not a second design.
- **`lib/icon-svg.test.ts` exists because nothing else reads the file.** `tsc`, `eslint`, `check:dates` and `build` all pass with a completely broken `icon.svg`, and a favicon has no error channel — a malformed one renders nothing, logs nothing, and leaves the previous icon in place, which is indistinguishable from a caching problem. The three tests guard exactly the two mistakes that cost an afternoon.

**Two silent failure modes, both of which broke this file once, both now tested**

- **An SVG served as a file is parsed as XML, and XML forbids `--` inside a comment.** The documentation comment in `icon.svg` named a CSS custom property by writing its two leading hyphens. The whole document failed to parse; Chrome drew nothing, logged nothing, and kept the Vercel icon — including in Incognito, which is what finally ruled out the cache. The parser error was only ever seen by rasterising the file with `qlmanage -t -s 256`, macOS's WebKit thumbnailer, which prints it straight into the thumbnail. No custom property may be spelled by name in that comment.
- **`var()` in a presentation attribute is not universally substituted.** `fill="var(--red)"` can fall back to black. Every colour is now set from the stylesheet, where substitution is guaranteed.

**Changed**

- `app/icon.svg` — new. The mark, with `@media (prefers-color-scheme: dark)` inside it, and a long comment carrying the geometry and the two XML rules above
- `app/apple-icon.tsx` — new. 180×180 through `next/og`, same coordinates at scale 4.375
- `app/favicon.ico` — deleted (25,931 bytes of Vercel triangle)
- `public/` — deleted: `file.svg`, `globe.svg`, `next.svg`, `vercel.svg`, `window.svg`
- `lib/icon-svg.test.ts` — new, 3 tests: no `--` in any XML comment, no `var(` inside a `fill=`/`stroke=` attribute, every `var()` token defined in both the light and dark blocks
- `proxy.ts` — `apple-icon` named in the matcher. Generated icon routes are extensionless, so a matcher that excludes static assets by file extension misses `/apple-icon` entirely and 307s it to `/login`; it was returning 105KB of HTML instead of a PNG. `favicon.ico` stays in the matcher though the file is gone, so probes get a fast 404 rather than a redirect
- `app/layout.tsx` — `suppressHydrationWarning` on `<body>` as well as `<html>`. React suppresses one level only, and the mismatch was `cz-shortcut-listen="true"`, injected by a browser extension
- `components/auth/SignInButtons.tsx` — a `pageshow` listener that clears `pending`. The back-forward cache restores React state verbatim: no mount, no effect, no fresh `useState`, so returning from the OAuth provider left the button disabled and mid-sentence

**State:** `npm test` **372**, `eslint`, `check:dates` and `build` all green. `/apple-icon` verified over HTTP as a 180×180 RGBA PNG. The mark itself was checked by rasterising `app/icon.svg` with `qlmanage` at every step, and the whole candidate set — 22 marks, each at 16px on cream and on `#1a1815` — was reviewed on the sheet before choosing. **Committed and pushed with this entry.**
...
## 2026-09-13 · The strip goes, its drawing stays · The table takes its own range

"Let's remove "Every habit, day by day" section, and make the graph in "When" in "Every habit, this month" look more like the graph in "Every habit, day by day".

For "Every habit, this month", let's remove that title. Internally we'll call it the table, but externally it doesn't need a name. Let the pagination at the bottom right tell you how many pages there are. Right now the column titles aren't perfectly on the same row. In fact, Marks is a bit above How Often, and they are bumping into each other and overlapping. When should not be sortable. The date range should be at the top left, and there should be a drop down next to it to choose the date range. The when section should be blocks for every day if the table is for a month, and for every month if the table is for longer than a year"

**What changed about the interface.** The eight-week strip is gone from the Habits tab, and its drawing moved into the table's own "When" column. That column was a row of hairline ticks; it is now blocks with gutters between them — faint when the stretch was empty, tinted when something landed, solid when you did it most of the days in it — which is the strip's vocabulary, one row per habit, in a cell. How long a block is follows the range: a day each up to a month, a week each up to a year, a month each past that, a year each past five. So the same drawing now answers "this week" and "all time", which is the thing the fixed eight weeks refused to do.

The table lost its title. It is the only table on the tab and it sits under its own thick rule, so what the top left carries now is the dates — "1 Sep — 13 Sep" — with a range dropdown immediately to their right. **That dropdown is the table's own.** The one up in the tabs band still governs the ranking, the star and the moods; this one governs the table and nothing else, so a month's ranking can sit above a year's rates, and each control prints the dates it resolved to right beside itself. The pager moved to the bottom right and now says "Page 1 of 2 · 13 habits" rather than "1–10 of 13": with no heading left, the line that says how much there is has to lead with how many pages there are.

Two header fixes. "When" is a plain label now — it is a shape, not a value, so there was nothing for a click to order by. And the headings sit on one line: the header row and the body row are built from one column definition, so the pad can't be on one and missing from the other, and every cell is middle-aligned, which stops a right-aligned header from taking its baseline off the chevron instead of off the word. That was the whole of "Marks is a bit above How Often, and they are bumping into each other."

The tab is two panels now instead of three: the ranking, capped, then the table, full width.

**Decisions**

- **The strip's drawing survived the strip, and that is the trade.** What is lost is a day-by-day grid of every habit's last fifty-six days that you could read across as a calendar. What is gained is a drawing that follows the range picker, sits in the same row as the count and the rate it explains, and no longer states the same "when" twice in two panels with two different windows. The tab had two answers to when, one of which ignored the control at the top of the page.
- **Blocks, not ticks.** A tick says *when* precisely and nothing about how much, it is 2px so there is nothing to point at, and a year of them piles into a smear. A block loses the exact day — which the "Last" column names anyway — and gains three things: a shade that means quantity, a hover hint, and a shape you can compare against the row above.
- **Four grains, chosen by how many blocks come out, not by what they are called.** ≤31 days a block a day (the picker's whole short half lands here), ≤366 a block a week (5 to 53, within one of the strip's 56, so the density is known to work), ≤1827 a block a month (13 to 61), above that a block a year. Past five years of months the blocks are thinner than the gaps between them, which is where a plot stops being blocks and becomes a texture.
- **The cuts are the calendar's, not the window's.** A weekly block starts on a Sunday and a monthly one on the 1st, so the first block of a window is usually a short one. Cutting into equal pieces from the window's own start is tidier arithmetic and puts the boundaries somewhere new every time the range moves by a day — two visits a week apart would draw the same history as two different pictures. Calendar cuts hold still, which is the whole basis for reading one row against another.
- **An edge block is measured by the days it actually covers.** "This month" on the 13th ends in a thirteen-day block; calling it thirty would draw a hard fortnight as a faint one. `PlotBlock.days` is clipped to the window at both ends and the shading reads that number.
- **Heavy means most of the days in the block, with a floor of two.** A single day still needs two marks, which is exactly the old strip's rule, so a day block means what it always meant; a week needs four and a month about fifteen. One mark per day of the block was the alternative and at week grain it asks for a perfect seven before anything darkens, which almost nothing clears — so every row draws the same.
- **Absolute, never relative to the row's own busiest block.** Scaling each row to itself makes every habit look equally consistent and takes away the one comparison a table of rows is for.
- **The table's range is independent, and that is a considered cost.** Two dropdowns on one page can say two different things, and someone will read the table's rates against the ranking's period. The defence is that they start on the same range and each prints its own resolved dates an inch from its own control. The gain is the reason the table exists: it is the panel you look something up in, and looking something up over a year while the ranking stays on this month is the normal thing to want.
- **No heading, and the `<caption>` picks up what it was carrying.** A title reading "Every habit" over the only table on the tab labels the only thing it could be labelling. A screen reader entering the table has none of the surrounding context, so the accessible name stays — it is just never drawn.
- **"When" is inert rather than disabled.** No button, no arrow, no hover. A disabled button would still be in the tab order announcing itself as a control you may not use; a header that does nothing should look like a label.
- **One column definition for both rows, which is a fix and not a tidy-up.** The overlap happened because the padding lived on the body cells and the alignment lived on the headers. An alignment declared twice is an alignment that will disagree again, so `cellClass` is the single place either row asks.
- **`spanLabel` and the caption builder moved to `components/trends/rangeText.ts`.** Two range pickers on one page have to format a period identically or the page is lying about one of them. Not into `lib/analytics.ts`, because it formats dates and `lib/dates.ts` reaches date-fns, which `node --test` cannot resolve.
- **The table resolves its own range, so `TrendsBoard` lost three memos and gained none.** It takes the library, the placements and today, and does `resolveBounds` → `habitWindow` → `habitTable` inside itself. The board keeps only what its own panels need.
- **`hits` became `HabitHit[]` — one entry per day, carrying that day's count.** It was an array of offsets, which was enough for a tick and not enough for a block: twice on a Tuesday has to be able to darken the block it falls in. The row count still counts placements and `days` still counts days, so nothing downstream changed meaning.
- **One walk to fill the blocks, not a lookup per mark.** `hits` is ascending and the blocks are built ascending, so the block index only moves forward. A hit outside the window is dropped rather than clamped onto the last block — it cannot happen from `habitTable`, and piling it on would draw a habit as busier than it was.
- **`nextBoundary` does month and year arithmetic on the string.** A `DayString` is big-endian and zero-padded, so the month is characters 5 and 6 and the next one is that number plus one. Going through `Date` to learn what follows January is the slower way to the same answer and puts a timezone back into a module that has spent four steps keeping them out.
- **`"first"` left the sort keys with the column that offered it.** Nothing else sorted by it, and the remaining four are the four the request named.

**Changed**

- `lib/habits.ts` — `HabitHit` replaces the bare offset; new plot layer: `plotGrain`, `plotBlocks`, `nextBoundary`, `blockLevel`, `PlotBlock`; `HabitSortKey` and `SORT_START` lose `"first"`
- `lib/habits.test.ts` — the fixture rows and the hit assertions follow `HabitHit`; six new suites over the plot: every grain boundary, calendar cuts at week/month/year grain across a year end, clipped edges, leap years, a covers-the-window-exactly invariant, and the shading thresholds
- `components/trends/HabitTable.tsx` — owns its `Range`; no heading; dates then `RangePicker` top left, area filter top right; `BlockPlot` replaces `TickPlot`; one `COLUMNS`/`cellClass` for header and body; "When" inert; the pager moved and re-worded
- `components/trends/rangeText.ts` — new: `spanLabel` and `captionFor`, shared by both range pickers
- `components/trends/TrendsBoard.tsx` — the heatmap and its two memos gone, the three habit-table memos gone, the three local formatters gone, the Habits tab restacked as a capped ranking over a full-width table
- `components/trends/HabitHeatmap.tsx`, `lib/heatmap.ts`, `lib/heatmap.test.ts` — deleted
- `lib/daymath.ts`, `lib/analytics.test.ts`, `README.md` — prose pointing at the deleted module

**State:** `tsc`, `eslint`, `npm run check:dates` and `npm run build` all green; **369 tests**, down from 372 — the strip's 29 went with it, and `lib/habits.test.ts` grew from 55 to 81 for the plot. The emitted stylesheet is 101,562 bytes and the new classes are all in it (`align-middle`, `w-[28%]`, `min-w-36`, `h-2.5`, `gap-0.5`, `justify-end`); a sweep of every comment added this pass found no utility spelled in prose that isn't also in markup. Verified by running the real functions over the demo year: all eight ranges cover their window exactly, with no gap and no overlap at any grain — "this week" resolves to a single day today (13 Sep 2026 is a Sunday) and draws one block; "this month" is 13 days and 13 blocks; "this year" is 256 days and 38 weekly blocks; "last 12 months" and "all time" are both 365 days and 53 weekly blocks, because the demo's first placement is 14 Sep 2025. Meditation reads 10 marks over 13 days of September and its blocks come out `1101101111110`. Committed — this entry is part of that commit.

**Open**

- **Still never seen in a browser.** Everything above is a build and a set of assertions. The block plot's whole claim is that it reads at a glance, and the two range dropdowns' whole cost is whether you can tell them apart — neither is verifiable from here.
- **Nothing goes heavy at day grain in the demo**, because the generator never places the same habit twice in a day, so a month of blocks is entirely faint-or-tinted. That is the old strip's behaviour exactly, and it is the right rule, but it means the three-level shading only earns its keep at week grain and above until real data has a double day in it.
- **Two range pickers on one page, and they look identical.** Same component, same width, same dropdown. If they turn out to be confusable, the fix is probably that the table's says something like "Table: this month" rather than that they differ in style.
- **The plot is `aria-hidden` with one `sr-only` sentence**, naming only the first mark. Thinner than the deleted strip's full `sr-only` list. Fine while the row states every other fact.
- **Nothing prints the block length.** A reader sees blocks and has to infer from the dates whether each is a day, a week or a month. The hover hint names the stretch, which is a hover away rather than on the page.
- **The demo has 13 habits and pages at 10**, so the second page holds three rows and the table visibly shortens when you go to it. Still worth seeing before deciding whether the body should hold its height.
- **One sign-in on the deployed site failed to load stickers and a reload cleared it.** Not reproducible, and the Vercel runtime logs had rolled by the time we went looking, so the error message was never seen. It is *probably* the `iat` race `lib/queries/settling.ts` exists for — but that module already waits 350ms + 900ms across three attempts, and the skew it covers has a ceiling of about a second, so a failure getting past it means either multi-second drift (widen `BACKOFF_MS`) or an error the deliberately narrow regex on line 48 never matched (something else entirely). Those need opposite fixes, so nothing was changed. **If it happens again, the one thing to capture is the Vercel runtime log line** — the throws are worded to be greppable: `Could not load placed stickers: <message>`, `Could not load moods: …`, `Could not load notes: …`. The part after the colon is the whole answer. The browser console's `digest` is a hash and does not carry the message, so the server log is the only place it exists.

---

## 2026-09-12 · Every habit, as a table · Five sortable columns, eight ranges, ten to a page

"I'd also like a view in the trends section that is like a table (but with the same styling, no background color, etc.) of all the habits, and then their area, their count, their approximate frequency (about every X days for example), a small inline plot with ticks representing each time they did it spaced out for the given duration, and then the date of the last time they did it (or 7 days ago for example), with a dropdown for this year, year to date, this month, last 30 days, this week, etc. Paginated, if the number of habits exceeds the screen length. And each column is sortable, except the habit column, by clicking the headline. The area should also be choosable (like limit to certain areas)."

**What changed about the interface.** The Habits tab has a third panel under the other two, running the full width: every habit you have, one per row, six columns. The habit, its life area with the area's dot, how many marks fell in the range, roughly how often that is — "About every 4 days", "About daily", "Once" — a strip of ticks showing when in the period those marks landed, and how long ago the last one was, read as "Today", "Yesterday", "3 days ago" rather than as a date you have to place yourself. Click any headline but the habit's and the table sorts by it; click the same one again and it flips, with a chevron on the active column and a faint one under the cursor on the others. A dropdown on the right limits it to the areas you tick. Ten rows to a page, with a pager that says "1–10 of 13" and only appears when there is a second page. It wears the same chrome as everything else on the page: one thick rule above it, hairlines between rows, no fill, no radius, no box. And the range dropdown that governs the whole tab grew from four options to eight — This week, Last 7 days, This month, Last 30 days, This year, Last 12 months, All time, Custom — so every panel on Trends can now be read over a week or over a rolling year.

**Decisions**

- **The range picker was extended rather than given a sibling.** The request asked for a dropdown on the table, and there is already one six inches above it governing every panel on the tab. A second one would mean two controls that both say "this month" and can disagree — and the four options it was missing are ones the other panels want too: a mood line over the last 30 days is a better question than a mood line over whatever fraction of September has happened. So the kinds went into `lib/analytics.ts` and the table reads the same `bounds` as the star and the tally.
- **Two families of range, and both are worth having.** *To-date* runs from the start of the period you are inside — this week, this month, this year — and answers "how is this going", at the cost of being three days long on the 3rd. *Trailing* counts a fixed number of days back from today, so it is always the same length and always comparable with itself. Offering only the first makes every rate jump on the 1st; offering only the second means the page can never speak about the month you are in. The user's own list asked for both halves in the same breath — "this year, year to date, this month, last 30 days" — so the dropdown reads as a zoom out with each to-date option directly above the trailing one of about its length.
- **Sunday, because the calendar already said so.** "This week" has to mean the same seven squares the week view highlights, and `weekGrid` takes date-fns' default start. The finder had to be written a second time in `lib/daymath.ts` rather than imported, because `lib/analytics.ts` runs under `node --test` where date-fns cannot be reached — so `startOfWeek` there is pinned against known Sundays, month ends and year ends, which is what stops the pair quietly disagreeing.
- **The frequency is the window over the count, not the gap between marks.** Four marks in the first week of a year-long window comes out "About every 91 days", not "About every 2". That reads oddly in isolation and it is the only version that doesn't make the row contradict itself: the count is in-range, the plot is in-range, and a rate measured over a different stretch than its neighbours can't be compared down the column. The column that tells you it stopped is the last one. The median gap between consecutive marks was the alternative — a better statistic for "how often when I'm doing it" — and it answers a question the range picker isn't asking, in a unit you have to know what a median is to read.
- **Under two marks there is no rate, so it doesn't state one.** One mark in a thirty-day window is "Once", not "every 30 days" — a rate implies a repetition that hasn't happened. Zero marks is an em-dash, `AreaTable`'s decision, for the same reason: zero is a measurement and a dash is "nothing here", and the eye skips a dash instead of reading it as a quantity. Anything that rounds to one day is "About daily", because "about every 1 days" is not a sentence and daily is the most a calendar of days can say.
- **Days all the way up, never weeks.** "About every 21 days" beside "about every 3 days" is two numbers in one unit you can read against each other; "about every 3 weeks" beside "about every 3 days" is two numbers that look alike and aren't.
- **The plot stretches and the strip above it doesn't, and that is the difference between them.** `HabitHeatmap`'s cells are fixed pixels because a cell has to line up with the cell in the row above; these ticks are only ever read against the other ticks in their own row, so the track is a percentage and the tick is 2px. It also means the plot works at any range the picker offers, including a year, which is the thing the eight-week strip deliberately refuses to do.
- **72 slots, so a year is a texture rather than a black bar.** Three hundred marks at 2px in a 200px column is a solid block that says nothing. Snapping each mark to one of 72 positions turns the long ranges into clusters and gaps and leaves the short ones untouched, since 72 is more than a month has days. A number rather than a measurement of the column, because measuring means an effect, a resize observer and a re-render per drag — 72 is a little denser than the narrowest this column gets, which is the error worth having.
- **A hairline through the middle of every plot, including the empty ones.** With nothing drawn on it, a row with no marks still has to read as a period that happened and was empty rather than as a cell that failed to render. The heatmap spends a repeating gradient on the same idea; at this density one line is the whole picture.
- **The habit column is the one that doesn't sort, and that is not a shortcut.** It is the row's label — a `<th scope="row">`, the way `AreaTable`'s area is — and what it would sort into, the alphabet, is a worse order than the one it arrives in: library order groups the table by life area, so the unsorted table is already banded by colour.
- **Every column starts at its useful end, which means the five arrows don't point the same way.** Clicking "Marks" to see your biggest counts is the reason you clicked; seeing your smallest is a second click. Frequency ascends, because the smallest interval is the most-done habit — the same intent pointed at a number that runs the other way. Getting that wrong is why `SORT_START` is a table rather than five guesses at a call site.
- **"When" sorts by the day the ticks begin.** A strip has no single value to sort on, and the header had to do something, so it sorts by the fact the drawing shows that no other column states: whether this is a habit you have been at all period or one that started last week. It is also what makes "every column except the habit" true literally rather than nearly.
- **Empty rows sink in both directions.** A habit with no marks has no frequency, no first day and no last day, and ascending by "last done" would otherwise open the table with a column of dashes. So the nulls go to the bottom whichever way the arrow points, and the arrow orders the rows that have something to say. Name breaks every tie, which is Step 12's stability rule: two equal counts coming out in a different order on different renders is how a reader stops trusting a panel.
- **A live habit always gets a row, even an empty one.** This is `heatmap`'s rule rather than the ranking's: "you have this habit and did none of it this month" is a reading, and a tracker that hides it is only ever showing you your wins. An *archived* habit gets a row only if it was actually placed inside the window — it isn't in the tray any more, so an empty row for it is a question nobody asked, while a month you spent doing it is still your month.
- **The area filter is checkboxes, and an empty set means all of them.** Checkboxes rather than a `Select` because it is the one control on the page whose answer can be several things at once — which is also why it looks different from the range picker, where a range is exactly one choice. Storing "all" as a full set is the bug worth avoiding: a set written when you had five areas keeps hiding the sixth you add later and nothing on screen explains why. Ticking every area collapses back to the empty set, so the trigger says "All areas" instead of "6 areas" for the identical table.
- **The page is clamped on read, not corrected in an effect.** Filtering to one area while on page 3 leaves the page number past the end, and the honest answer is the last page — the rows are still there, you just aren't looking at them. Clamping where the page is used means no effect that resets state and no render where the table is briefly empty. A re-sort does reset to page 1, because the row you were looking at is not on page 2 of a different order.
- **The pager is hidden at one page, not disabled.** A greyed-out pager under four rows is furniture explaining a limit you haven't reached. It says "1–10 of 13" rather than "Page 1 of 2", because the question a reader has is how many habits there are; the page number is only interesting as a way to the rest.
- **Ten rows, a number rather than a measurement.** A table that pages at whatever height the window happens to be shows a different amount when you resize, and every page then holds a different amount of what you were comparing.
- **The panel is full width under the pair, not a third peer beside them.** The two above are drawings you read at a glance and they sit side by side at a glance's width; this is six columns you look something up in, and a table squeezed into a 20rem track is a table that wraps. Below them, because the order is coarse-to-fine: which habits you did most, then what each looks like day by day, then the numbers behind both.
- **It overlaps both of its neighbours and is kept anyway.** The count is in `MostDone` and the ticks are a coarser `HabitHeatmap`. What is new is the combination — a rate, a last-done, and a row per habit rather than a top eight — and the direction you read it: the two drawings are for noticing, this is for looking something up.
- **The plot is `aria-hidden` and the cell is not empty.** Every fact in the drawing is also in the row it sits in, which is what makes hiding it honest — but a cell that announces as blank under a heading is its own bug, so the cell carries "First on 2 Sep" for a screen reader. That is also what the column sorts by, so the sentence and the header agree.
- **The arithmetic is a bundler-free module with 55 tests, and the component holds none of it.** `lib/habits.ts` imports one function from `lib/daymath.ts` and nothing else, which is what lets `node --test` reach it — the rule `lib/analytics.ts` wrote down and `lib/heatmap.ts` repeated. What lives there is every judgement worth being wrong about: which rows exist, what the interval means, where the nulls sort, how a page is sliced. The component is layout, and `sortHabits` is a separate function from `habitTable` precisely so a header click doesn't re-walk the placement map.

**Changed**

- `lib/analytics.ts` — `Range` grows `week`, `days7`, `days30` and `days365`; `RANGE_LABEL`, `RANGE_KINDS` (in zoom-out order), `resolveBounds` and `rangePhrase` follow, with the trailing ranges reaching back one day less than they are long because they include today
- `lib/daymath.ts` — `weekday` and `startOfWeek`, UTC-only like everything else in there, so `lib/analytics.ts` can find a Sunday without date-fns
- `lib/habits.ts` — new: `habitWindow` and `earliestPlacement` (which close `Bounds`' open sides into a real span), `habitTable`, `frequencyLabel`, `sortHabits` with `SORT_START`, `pageCount`/`pageOf`/`HABITS_PER_PAGE`
- `lib/habits.test.ts` — new: 55 tests, most of them about the judgements rather than the arithmetic
- `lib/analytics.test.ts` — the four new kinds, including that "this year" and "last 12 months" are genuinely different ranges, and a loop asserting every kind the dropdown offers has both a label and a phrase
- `lib/daymath.test.ts` — `weekday` numbered from Sunday across a full week, `startOfWeek` across a month end and a year end, and a 400-day sweep asserting it never moves a day more than six squares
- `components/trends/HabitTable.tsx` — new: the panel, the sortable headers, the tick plot, the area filter and the pager
- `components/trends/TrendsBoard.tsx` — three memos (`earliestPlacement`, `habitWindow`, `habitTable`) and the panel mounted full width under the Habits grid, plus its own caption builder

**State:** `tsc`, `eslint`, `npm run check:dates` and `npm run build` all green; **372 tests, up from 303** — 55 in `lib/habits.test.ts`, the rest split between the new ranges and the week finder. The emitted stylesheet is 98,945 bytes of utilities plus a 3,391-byte font chunk, and the new classes are all in it (`min-w-2xl`, `w-[30%]`, `h-3.5`, `size-3.5`, `text-[0.9rem]`, `hover:text-ink`, `group-hover:opacity-40`, `before:content-['']`); a sweep of every comment added this pass found no utility spellings emitting dead rules. Verified by running the real functions over the demo year: 13 rows in every range, so the pager is exercised at 2 pages; "This week" and "Last 7 days" resolve identically today because it is a Saturday; "All time" opens at 2025-09-13, the demo's first placement, rather than at the epoch; Meditation reads 6 marks and "About daily" over the week, 21 and "About daily" over 30 days, 157 and "About every 2 days" over 12 months; a year of 157 marks snaps to 64 of the 72 tick slots with none out of range; and page 3 of a two-row area filter clamps to the rows rather than going blank. Committed — this entry is part of that commit.

**Open**

- **Still never seen in a browser** — the extension has been unavailable for four sessions, and this is the first panel with real interaction in it: a sort that flips, a menu that stays open across clicks, a pager, and a chevron that appears on hover. None of that is verifiable from a build. The Habits tab is client state on top of a client-state tab, so `/demo`'s HTML never contains it either.
- **Sorting by "How often" is almost the same order as sorting by "Marks"**, and in a fixed window it is exactly the same order — the interval is the window divided by the count. The two differ only in where the single-mark and empty rows land. Both columns sort because the request said every column does; whether the frequency header earns its click is a real question, and the answer if not is the median gap, which is the statistic the decision above declined for other reasons.
- **The demo has 13 habits and pages at 10**, so the second page holds three rows and the table visibly shortens when you go to it. Worth seeing before deciding whether the body should hold its height.
- **The tick plot is `aria-hidden` with one `sr-only` sentence standing in**, which is thinner than the heatmap's full `sr-only` list. Fine while the row states every other fact; worth revisiting if the plot ever says something the row doesn't.
- **Nothing prints the window's length**, so "About every 4 days" over 255 days and over 30 days look identical in the cell. The dates are in the panel's heading and beside the range picker, which is two places, and the reason that might not be enough is exactly the reason the frequency is measured over the window.

---

## 2026-09-12 · A second pass over the same surface · The tray follows the view · Nothing is a card

"Let's make the scrollbar more custom for the app's styling, and make the sticker tray a touch wider to accommodate the scrollbar. And the scroll part should be below the 'Add Stickers' and add button, that part doesn't need to scroll. The stickers on the week view in the sticker tray should also be rectangular, and the rectangular stickers should have the white border and drop shadow too. Make the border-radius a bit smaller than it is presenting now, and allow a bit more breathing room between stickers on the week view. Let's also have the headlines for day of week (mon / tues / wed) throughout be the Space mono used for eyebrow headings. And for the week view, let's try a style that has no background, a thick border above, no left and right and bottom border, but thinner border between days of the week. Let's also have the month view have no background color."

"The sticker tray for the month should keep the circles, just the sticker for the week is rectangles. So like, everything matches."

"Let's update the stylings on the Trends section to match that of Week now. And the styling for the Month could be updated for that of the Week. For example, the one thicker border and other thin borders --- add that to Month. And remove the backgrounds on the Trends section and remove the border radiuses, and leave some borders out."

**What changed about the interface.** The tray draws whatever the calendar next to it is drawing. In the month it is the list of names it always was — a circle, then the habit beside it. Switch to the week and every row becomes the same rounded rectangle the week strip stacks: tinted, white-rimmed, sitting on its own shadow, with the name printed inside it and the tint filling the whole row. The eye stays on the right in both. The heading and its `+` no longer move: they sit still at the top of the rail and only the stickers under them scroll, down a slim ink-coloured scrollbar drawn to match the app, and the rail widened by a rem to pay for the width that bar permanently takes. Then the cards went away. The week lost its outline first — one thick rule above it, hairlines between the seven days, no sides and no bottom, so the columns hang off the heading and end wherever their notes end — and the month and all five Trends panels followed it: no fill, no radius, no box, a thick rule saying a new thing starts here and hairlines inside saying it has parts. The two empty states are the same rule, dashed. Both grids lost their fills too: the month's cells are the page's cream now rather than white, with the out-of-month days still recessed. Day names — the SUN–SAT row over the month and the one above each week column — are in Space Mono with everything else that labels something. And the week's bars breathe: 6px apart instead of 2, with a slightly tighter corner than before.

**Decisions**

- **The scrollbar had to be *drawn* before it could be styled, and that is the whole reason this change exists.** macOS overlays its scrollbar and fades it out, so the rail was a list that scrolled with nothing anywhere saying so. Styling `::-webkit-scrollbar` opts the element out of overlay behaviour: the bar becomes permanent and takes real layout width. That cost is the feature — it's what makes the rail look like a thing with more in it than fits.
- **The standard properties and the WebKit pseudo-elements cannot both be used, so the utility picks one per engine.** Blink and WebKit ignore `::-webkit-scrollbar` entirely if `scrollbar-width` or `scrollbar-color` is set to anything but `auto`, and both of those inherit down the tree from `html`. So `scrollbar-app` hands them back to `auto` *inside* `@supports selector(::-webkit-scrollbar)` and draws the pseudo-elements — while Firefox, which matches no such selector, never sees the reset and keeps the thin inherited ink bar it already renders well.
- **A whole rem for the rail, not eight pixels.** 13rem → 14rem, against a bar 8px wide. The extra four are because the thumb is inset by a transparent border rather than filling its track, so a name ending exactly at the track's edge still reads as touching the bar. The rail's furniture arithmetic is unchanged otherwise, and `MoodPicker`'s note now carries the number the five mood columns actually get.
- **The heading is the part that never scrolls, because it is the part you reach for when the list is long.** The `+` that makes a sticker and the line that says what the tray is for ("Drag one onto a day", or the clear-highlight link when something is lit) are the two things in the rail with nothing to do with any particular sticker. Scrolling past them to add a sticker is the specific annoyance; everything below is what the scroll is *for*. A capped flex column with the header fixed and the list allowed to shrink past its content is what splits them — and the list's minimum has to be explicitly zero, or a flex item refuses to shrink below its contents and the whole column grows instead of scrolling.
- **A tray row is drawn in whichever shape the calendar is drawing, and the first attempt got that wrong.** The question looked like "where does the name go" — a circle with the name beside it, or the week's rectangle with the name on it — and the rail is where you pick a sticker up, so a sticker about to be dragged onto a day should already look like what it will be once it's there. That argument is sound and it is *relative to the view*: in the month what lands on a day is a circle, so a bar in the rail promises the wrong object. Both shapes are right, one each. The rail and the grid now always agree, which is also already true of the copy under the cursor — `CalendarBoard`'s overlay has switched on the view since the week landed.
- **One read of the view for the whole rail, not one per row.** `StickerTray` asks `useCalendarView()` once and hands every row a `"mark" | "bar"`, the same two words `DraggableMark` uses for the same two shapes. Fifteen rows reading the context separately cannot disagree in practice, but a rail with one row in the other shape is the bug worth making unrepresentable — and the prop is required rather than optional, because a default nothing passes is a branch you cannot reach by using the app. The band survives either way: a lit row still washes behind its bar, which is the same picture a lit Tuesday makes with a mark sitting on it, and `wash()`'s 35% is already the number that keeps those two colours apart.
- **Archived rows take the same shape as the live ones, faded.** A drawer of circles under a list of bars would read as archiving having changed what a sticker *is*. What says these are retired is the fade and the missing verbs, so the shape follows the view with everything else.
- **`TrayRowFace` survived, and it is now the month's tray row and the day modal.** The modal keeps it in *both* views, which is the interesting half: that list is tick boxes, and a tinted bar next to a checkbox reads as a control inside a control when the checkbox is already the thing you press. It also keeps the `aria-hidden` wrapper that stops a screen reader saying "Gym, Gym" — a problem the bar cannot have, because there the picture and the word are one element.
- **The rectangle's radius came down and the circle's did not change.** 6.4px on a 26px circle is nothing; on a 26px-tall rectangle it is most of the way to a pill, and a die-cut sticker should read as cut from a sheet rather than stamped out. 4.8px keeps the corner soft and lets the shape stay a rectangle. The draggable's focus ring had to follow the same number, or the ring cuts the corner it is tracing.
- **6px between bars in a week column, up from 2.** Two was right for bars that were flat tint with a transparent border — they read as one block of colour with hairlines through it, and tightening them was how you saw the block. They are objects now, and 2px put each sticker inside the previous one's shadow: the rim's whole job is to say where one sticker stops, which it cannot do with nothing after it. 6px is the shadow's own reach plus enough to read as a gap, and it costs about one sticker's worth of the stack's floor before a busy day starts making the column taller.
- **`daylabel` moved to the mono, reversing the call in the entry below it.** That exemption was real — SUN–SAT are seven words being scanned, not seven tags — and it stopped being right in the same request that took the week's box away. With no card around the week, the name above each column is the only thing labelling it, and a label that isn't in the label face reads as content. Sentence case stays for the original reason: these are still words, so they are not being shouted.
- **The week stopped being a card, and within the hour so did everything else.** The first argument was a distinction: forty-two cells have to hold together as one object, while seven tall columns with their own name, numeral, stack and paragraph are seven separate things, and one outline around those read as a card with seven panels. So the week got a rule above and nothing else — and the page got quiet enough that the distinction stopped being worth keeping. Two views of the same seven columns speaking two different chrome languages is worse than either one everywhere, so the month's border and radius went too. It keeps its hairlines, which are the part that was holding it together.
- **Trends was five white boxes on a cream page, and there are only three answers on it.** A rounded filled panel is a claim that something separate is going on inside it, and five of them tile the tab with objects rather than with readings. They are all `PANEL` now: one 2px rule, no fill, no radius, no side or bottom edges. Two of them side by side in a grid read as one rule broken by the gap between them, which is how the week reads across its seven columns — that is the shape being copied, not just the border width.
- **"Leave some borders out" is the thick-and-thin grammar, and the thin ones stay.** Thick says a new thing starts here; thin says it has parts. So `AreaTable`'s rows keep their hairline dividers and the month keeps its 1px grid, and what got dropped is every line that only drew a box: the sides, the bottoms, the radii. The two empty states — Trends with no data, and the unbuilt-tab placeholder — are the same rule dashed, since dashed is already how this app says "nothing here yet".
- **The chrome lives in `lib/layout.ts` now, for the `TRAY_INSET` reason, and it is two constants rather than one.** Ten files were about to spell the same border width and token, and chrome that agrees by coincidence drifts the first time one site is tuned. `RULE` is the line itself — 2px of `--rule` along the top and nowhere else. `PANEL` is that plus the air a heading needs under it. The split is not tidiness: the two grids put their cells hard against the rule and the two empty states set their own padding around a centred sentence, so only the Trends panels want the `pt-4`. Both empty states are `RULE` composed with `border-dashed`, which works because they are different properties — no conflict and no order to get wrong. `PANEL` carries no horizontal padding either, because that would indent every panel's text from the page gutter `PAGE_WIDTH` exists to keep.
- **The heatmap's sticky name column had to change colour to stay invisible.** It is an opaque strip that the day squares scroll behind, so its fill has to be whatever it is lying on — and with the panel's white gone, that is the page's own cream. Left alone it would have been a white rectangle down the left of a cream panel, visible only while scrolling, which is the kind of bug that ships.
- **That rule is its own token, because ink at a slash opacity compiles wrong.** Lightning CSS renders a dropped `color-mix` as its first argument at full opacity, so a 20%-ink border degrades to a near-black 2px bar across the top of every week — and, after the second correction, of every panel in the app — on any browser without `color-mix`. Caught in the emitted stylesheet, not by reasoning: the fallback rule was sitting there ahead of the mix. `--rule` is a literal `rgb(… / …)` in both themes instead — the same answer the sticker shadow already needed. A hairline was the other option and 2px of hairline is a smudge.
- **"No background" means the cells are the page's cream, not that they have none.** Both grids draw their lines as 1px gaps showing the container through them, so a transparent cell shows hairline colour across its whole face rather than only in the gaps — and `wash()` is translucent, so a lit day needs something opaque underneath it either way. The fill moved from `--surface` to `--bg` in both grids and in the skeleton.
- **Out-of-month days kept their distinction for free.** They were never white: `--surface-sunken` is a shade under the page cream in light and under the card in dark, so with the white gone they still read as recessed — and they are now the only fill left in the month.
- **Tailwind scans markdown, and this file was emitting live CSS.** A `.lg\:w-72` rule that matched nothing in the source traced to the changelog entry below, which spells old class names in prose. Two `@source not` lines exclude this file and the README; that removed 21 dead rules — stale rail widths, the old page cap, the old eyebrow tracking, a `grid-cols-8` nothing has ever used — for 1,675 bytes, with nothing newly missing. The same trap fired twice more inside code comments written during this pass, both caught the same way and both reworded to describe a utility rather than spell it.
- **The skeleton was two narrowings stale.** It still reserved 18rem for a rail that had been 13 and was about to be 14, so the whole board jumped 64px sideways the moment real data replaced it. Which is the point of a skeleton being wrong: it is the one component nothing looks at while it works.

**Changed**

- `app/globals.css` — `@source not` for the two markdown files; `--scroll-thumb`, `--scroll-thumb-hover` and `--rule` on both themes, with `--rule` bridged as `border-rule`; the new `scrollbar-app` utility; `html`'s own scrollbar de-mixed to the thumb token over a transparent track; `daylabel` repointed at `--font-mono` (and its header note, plus the eyebrow's closing paragraph, rewritten — both asserted the exemption this pass reversed)
- `components/calendar/StickerBar.tsx` — the flat tint becomes an object: the transparent border it was already holding turns into `border-[1.5px] border-sticker-edge shadow-sticker`, at the tighter radius
- `components/dnd/DraggableMark.tsx` — the bar's focus ring follows that radius and fills the column
- `lib/layout.ts` — `RULE`, the 2px line on its own, and `PANEL`, which is that plus `pt-4`; both with the argument for no fill, no radius and no side or bottom edges attached
- `components/calendar/WeekGrid.tsx` — the card becomes one `RULE`; `bg-hairline` stays, because the hairlines are what it is showing through the gaps
- `components/calendar/WeekDayColumn.tsx` — column fill to `bg-background`; the stack's gap 2px → 6px
- `components/calendar/MonthGrid.tsx`, `components/calendar/DayCell.tsx` — label cells and in-month cells to `bg-background`, the sunken rung untouched; then the frame and its radius go, leaving the week's `RULE` over the same hairline grid
- `components/tray/TrayGroup.tsx` — a `RowShape` of `"mark" | "bar"` and a `RowBody` that switches on it, shared by the live rows and the archived ones; `TrayRowFace` stays for the month's rows and the day modal
- `components/dnd/DraggableSticker.tsx` — not a single class changed, and the eight added lines say why: the face's gap is inert under a bar rather than wrong, and a 26px bar is exactly as tall as a 26px circle, so nothing in the rail moves when the view switches
- `components/tray/StickerTray.tsx` — header pinned, the sticker list scrolls with `scrollbar-app`; one `useCalendarView()` read for the whole rail, threaded to every row and to the archive drawer; `StickerMark` import dropped
- `components/dnd/CalendarBoard.tsx` — the aside is a capped flex column at 14rem, with the status line pinned below the scroll
- `components/views/CalendarSkeleton.tsx` — 18rem → 14rem, the same fills as the real grid, and the same frameless grid now that the month has no box
- `components/trends/ChartCard.tsx`, `MostDone.tsx`, `MoodLine.tsx`, `HabitHeatmap.tsx`, `Readout.tsx` — five rounded white cards become five `PANEL`s; the heatmap's sticky name column moves from the card's white to the page's cream
- `components/trends/TrendsBoard.tsx`, `components/views/Placeholder.tsx` — both empty states keep the dash and lose the box: `RULE` with `border-dashed`
- `components/views/TrendsSkeleton.tsx` — the chart's grey block squares off, since the panel it sits in no longer has a corner
- `components/tray/MoodPicker.tsx` — the column arithmetic re-stated for a 14rem rail minus the scrollbar
- `PROGRESS.md` — the entry below corrected where it said the page cap had been removed rather than raised

**State:** `tsc`, `eslint`, `npm run check:dates` and `npm run build` all green; 303 tests, unchanged — none of this is testable arithmetic. Verified as far as the build reaches: the stylesheet carries `.scrollbar-app::-webkit-scrollbar{width:.5rem;height:.5rem}` with the standard-property reset inside its `@supports`, `--scroll-thumb` and `--rule` resolving to literal alphas in both themes, `.border-rule` pointing at the token with no `color-mix` fallback ahead of it, `daylabel` on the mono, and the 96rem cap still resolving to exactly one rule. After the card pass it is 101,386 bytes: no `rounded-2xl` at all, and the one surviving `rounded-xl` is the dialog's, which is deliberate — a modal is a floating card and still looks like one. `overflow-hidden`, `rounded-md` and `border-hairline` all still have real users elsewhere, so nothing went dead. `/demo` renders the rail at 14rem with a pinned header over a scrolling list, and — because the month is what the server draws — **83 circles and no bars**: 70 on the grid and 13 in the tray, which is the correction working. The forty-two cells split 30 cream and 12 sunken under a single `border-t-2 border-rule` with no frame, and seven mono day labels above them. Committed as `e5fcf07`.

**Open**

- **Still never seen in a browser** — three sessions now without the extension, and this pass is the one that most needs eyes: a drawn scrollbar, a rim against a wash, and a grid whose lines are the only thing left holding it together are all judgements a stylesheet cannot make for you.
- **The week view and the Trends panels cannot be verified at all from the server.** Month/Week is a hash route, so the rendered HTML is always the month, and the Trends tab mounts client-side too — the demo page emits exactly one `border-t-2 border-rule`, which is the month's. Everything above about the week's bars and the five panels' new chrome is read off the source and the compiled CSS.
- **The name on a tray bar is 0.78rem**, a rung down from the 0.83 the month's rows still use, because the bar is the week strip's component and the two have to stay identical. In a 14rem rail that leaves about 130px of name — and it means switching Month/Week changes the tray's type size, which is either invisible or the next thing to fix.
- **Firefox gets a different scrollbar** — the thin inherited one, not the drawn one. That is deliberate and worth looking at once.
- **Nothing on the page has a bottom edge now**, and on a tall screen that may read as unfinished rather than as open. It was one open shape when only the week had it; it is the whole app's grammar as of this pass, so if it reads wrong it reads wrong everywhere at once — which is the good case for finding out, and the reason to look at Trends and the month side by side rather than one at a time.
- **`RULE` has ten tenants and no story for a panel that needs a fill.** The next one that does — something that has to look inset, or a panel over a coloured band — either forks the constant or breaks the pattern, and that is worth deciding deliberately rather than by appending a background at the call site.
- **The wash behind a bar** is the one colour pairing designed for a mark on a cell rather than for a rectangle inside a band, and a lit tray row in the week view is now the place those two meet most.

---

## 2026-09-12 · Six passes over the surface · The labels get their own face

"Let's try Space Mono for the little eyebrow headings. Add some weight to the Instrument Serif headings. Add a white border to the stickers with a light drop shadow to indicate 'stickers'. Let's remove the 'Good / bad' names for the moods and let the icons carry it. Let's try to decrease the width of the sticker tray as much as possible to give more room to the week/month view, and let the entire body expand when there is room on the user's screen"

**What changed about the interface.** The small caps that name things — MOOD, WORK & STUDY, MOST DONE, the column heads in Trends — are set in Space Mono now, a touch smaller and a lot less letterspaced. Every sticker has a white rim around it and sits on a soft shadow, so a coloured circle reads as something placed on the day rather than printed into it. The five moods are five faces with nothing written under them. The tray is 80px narrower, and the page stretches further before it stops: the cap moved from 72rem to 96rem, so on a wide screen the calendar keeps going to about 1536px instead of halting at 1152 with margins either side.

**Decisions**

- **The headings did not get weight, because there is none to get.** Instrument Serif ships 400 and nothing else — confirmed against Next's own `font-data.json`, not from memory. `font-semibold` on a heading gets a *synthetic* bold: the browser smearing the outline sideways, which on a high-contrast serif reads as a printing fault, and `app/layout.tsx` has said so since the face landed. The three real options were a variable serif with actual weights (Fraunces, Playfair), optically thickening Instrument Serif with `-webkit-text-stroke`, or leaving it. Left it, on the call that swapping the display face to win one weight is a bigger change than the one being asked for.
- **A mono, and not just a smaller sans, for the eyebrows.** An eyebrow is a tag, not writing. Every glyph on the same advance is what makes a column of them read as a system of labels; it also puts real distance between the three faces, so a screen wearing all of them has three visibly different jobs rather than three shades of one.
- **Space Mono arrives smaller and much tighter, and both corrections are the same fact.** A mono's advance is the widest glyph's, so every letter comes surrounded by air it didn't ask for. That reads a rung bigger than DM Sans at the same nominal size (so 0.645rem → 0.6rem) and it has the letterspacing built in already (so 0.18em → 0.08em). Keeping the old tracking on top of a mono reads as an ellipsis between letters rather than as a label — and it spends width the tray no longer has.
- **`daylabel` stayed in DM Sans, and now for a second reason.** SUN–SAT was already the one place the eyebrow family is deliberately broken, because those are seven words being scanned rather than seven tags. A mono would have made that worse in the same direction.
- **The sticker rim is the white of backing paper, hard-coded, not `--surface`.** A die cut leaves a margin of the sheet around the art, and that rim is the most recognisable thing about the object — more than the gloss. It can't be `--surface`: that is the same colour as the tray's card, where the rim would vanish, and different from a sunken grid cell, which is where a sticker actually sits. So the rim is the paper's colour rather than whatever it is lying on.
- **1.5px, not 1 and not 2.** One pixel of rim around a 26px circle is legible one sticker at a time and disappears in a grid — the exact mistake the old hue ring made, and the reason it was dropped in Step 19. At 2px the rim starts competing with the fill for the circle and the mark inside loses its ground.
- **Nothing reflowed.** The border was already in `StickerMark`'s class list at 1px and transparent, left there two steps ago precisely to hold the 26px border-box so a visible edge would cost no layout later. This is that cheque being cashed: 1.5px of the box is now paper instead of nothing, and every row of marks lays out where it did.
- **The shadow is a literal alpha and the one token here that does *not* derive from `--ink`.** Everything else in `globals.css` wants to flip with the theme; a shadow does not. Ink is `#1c1a17` in light and `#f0ebe0` in dark, so an ink-derived shadow becomes a pale glow in dark mode — under a rim that is already the lightest thing in the sticker, that reads as a halo rather than a lift. First draft got this wrong and the built CSS is what caught it. `.dark` overrides with true black at roughly triple the alpha, because a shadow on a `#26231f` cell has almost no contrast to work with.
- **Which also dodged a fallback.** Lightning CSS renders a dropped `color-mix` as its first argument at full opacity. On the `--ramp-*-tint` values that is invisible; on a shadow it would have been a heavy black ring under every sticker on any browser without `color-mix`.
- **Losing the mood words meant *adding* back an accessible name, and that was the load-bearing half.** The labels were wrapped over an `aria-hidden` face, on the reasoning that the visible word already named the button and `MoodMark`'s own `sr-only` "Mood: Great" would otherwise read twice. Delete the word and that wrapper turns five buttons into five identical unnamed ones. So the fix was to stop hiding the mark rather than to add anything — `MoodMark` was already carrying the name for the case it meets on a date line in the grid.
- **The words were also what set the tray's floor.** Five labelled columns wanted ~54px each; five bare faces want 37. With them gone the binding constraint becomes a sticker row — 16px of inset, a 26px mark, a 10px gap and a ~24px eye, so 76px of furniture — and 13rem leaves ~130px for a name, about 19 characters against a `NAME_MAX` of 24. So the longest possible name truncates and a typical one does not. `TrayGroup` has always truncated; a 24-character name was already clipping at 18rem.
- **The page cap was raised, not removed.** The old argument for 72rem was that a month is a fixed seven columns, so it doesn't need width — it just fills it. True of the month, wrong about the week, which is seven stacks of *named* bars plus a note field, where every extra pixel is a habit name that doesn't truncate. The month gets bigger squares out of the same deal, which is no worse than bigger margins. It went to no cap at all first, and that was one step too far — a month on a 34" monitor is seven very wide columns of mostly empty cell.
- **96rem, and written as the breakpoint variable rather than as a number.** Tailwind's max-width scale stops at 7xl (80rem/1280px), so a wider cap has to come from the breakpoint scale, and taking it from there keeps the cap and the point where `2xl:` utilities start firing as *one* number. A cap at a round 1500 would mean every `2xl:` rule engaged 36px after the layout had stopped growing. 1536 is conventional on its own terms — 12 × 128, 16 × 96 — and the runner-up was 1440 (90rem), which is the commoner number in design files and has no Tailwind token, so it would have been a bracketed magic number.
- **Tailwind's scanner reads comments.** The first draft of that note in `lib/layout.ts` spelled out the two alternative class names in prose, and the build emitted live CSS rules for both — the scanner cannot tell documentation from markup. Two dead rules, caught by grepping the emitted stylesheet for every `max-w` it contained. The alternatives are now described rather than spelled, and the file says why.

**Changed**

- `app/layout.tsx` — `Space_Mono` at 400 only (not a variable font, so the `weight` array is required rather than optional), a third `--font-*` variable on `<html>`, and the two-faces note rewritten as three
- `app/globals.css` — `--font-mono` repointed at Space Mono (the token was unoccupied; nothing in the app wore `font-mono`); `eyebrow` gains a family and loses size and tracking; `--sticker-edge` and `--sticker-shadow` on both themes, bridged as `border-sticker-edge` and `shadow-sticker`
- `components/calendar/StickerMark.tsx` — the transparent border becomes `border-[1.5px] border-sticker-edge shadow-sticker`
- `components/tray/MoodPicker.tsx` — the label span and the `aria-hidden` wrapper both go, `flex-col` with it; `MOOD_LABEL` import dropped (still exported, still used in six other places)
- `components/dnd/CalendarBoard.tsx` — the rail `lg:w-72` → `lg:w-52`
- `lib/layout.ts` — `PAGE_WIDTH`'s cap goes from 72rem to 96rem, as the breakpoint variable
- `components/tray/StickerTray.tsx` — two stale "18rem" in the grid note

**State:** `tsc`, `eslint`, `npm run check:dates` and `npm run build` all green. 303 tests, unchanged — nothing here is testable arithmetic. Verified past the build as far as it goes without a browser: the emitted stylesheet carries `.eyebrow{font-family:var(--font-mono)…}`, `.border-sticker-edge`, and `.shadow-sticker`, with the two shadows resolving to `#1c1a1729`/`#1c1a1717` in light and `#00000080`/`#0000004d` in dark and no `color-mix` left in either; `/demo` renders 83 stickers wearing both new classes, seven eyebrows, `lg:w-52` present, the 96rem cap resolving to one `max-width:var(--breakpoint-2xl)` rule and no stray ones beside it, all three font variables on `<html>`, and the five mood buttons containing nothing but `<span class="sr-only">Mood: …</span>`. Uncommitted.

**Open**

- **Never seen in a browser.** The Chrome extension has now been unavailable across two sessions. Everything above is types, lint, tests, a build, and grep over the emitted CSS and HTML — which cannot tell you whether Space Mono at 0.6rem/0.08em actually *looks* like a label, whether 1.5px of rim reads at 26px, or whether the shadow is too much at a month's density.
- **Space Mono is wider per character than DM Sans even after the corrections**, and the longest eyebrows sit in the narrowest place: `TrayGroup` truncates its label, and the rail just lost 80px. "ROMANCE & ADVENTURE" in the 13rem rail is the case to look at first.
- **The naked mood faces are a 20px hit target** in a 37px column. Fine with a mouse; worth checking with a thumb, where the labels were part of what made the row easy to hit.
- **Whether 1536 is the right stop.** It is 36px past the round 1500 that was asked for, taken to keep the cap on the breakpoint. 1440 is the fallback if it reads too wide, and the reasoning for both is written down where the constant is.
- **Prose measure at the cap.** The two places that hold sentences — a day's note and "Drag one onto a day" — have no `max-w` of their own. At 1536px neither reaches a bad measure, and the cap is now what guarantees that rather than luck.
- **The white rim against `wash()`.** A highlighted row tints the background behind the stickers; the rim was designed against the card and the sunken cell, not against that.
...
- **Emoji still render.** A mark that isn't `icon:`-prefixed falls through to the old path. That's what makes the migration a `UPDATE` you can run whenever (`npm run adopt:icons`) rather than a flag day. **Since run** — the existing rows are converted.
