...
import { CalendarDataProvider } from "@/components/calendar/calendarData";
import { CalendarViewProvider } from "@/components/calendar/viewMode";
import { PAGE_WIDTH } from "@/lib/layout";
import { isCalendar, pageFromHash, type Page } from "@/lib/nav";
...
export function AppShell(props: Props) {
  ...
  useEffect(() => {
    const readLocation = () => setPage(pageFromHash(window.location.hash));
    readLocation();
  ...
  return (
    ...
    <CalendarDataProvider>
      ...
      >
        {props.banner}
        <TopNav page={page} onPageChange={changePage} end={props.navEnd} />
      {/* 40px of top padding and not the old 56px. The target is a 14"
          ...
          rather than left for the page to collide with — 63px of bar (a 46px
          pill, 8px above and below it, and the hairline) plus the page's own
          40px is 6.5rem. Above 64rem there is no bar and the bottom
          goes back to matching the top. */}
        <main className={`${PAGE_WIDTH} flex-1 pt-5 pb-26 lg:pt-10 lg:pb-10`}>
          {/* Both landings are calendar sections, so null is a calendar. */}
          {page === null || isCalendar(page) ? props.calendar : props.trends}
        </main>

        <BottomNav page={page} onPageChange={changePage} />
      </CalendarViewProvider>
    </CalendarDataProvider>
  );
}
