 ...
 * from a tally.
 *
 * The donut is gone, and with it `circumference`, `donutArcs`, `sliceMidAngles`
 * and `stackOffset` — dash-pattern arithmetic and two-line label lifting that
 * nothing else was ever going to want. `VIEW` and `LINE` stay exactly as the
 * donut left them: the box was budgeted for its horizontal labels, which are
 * wider than the star's, and shrinking it now would move the one card still on
 * the page for no reason but tidiness.
 *
 ...
 * lands on `radius` too, back when there was a ring.
 *
 ...
 *
 * SVG has no line box — a `<tspan>` moves by exactly the `dy` you give it — so
 * leading here is a number somebody picks, and 14 against a 12-unit font is the
 * usual ~1.2.
 *
 * Nothing draws with it any more; the donut that stacked a percentage under a
 * name was the only caller. It survives as the unit `VIEW.height`'s budget is
 * written in, and the assertion in `charts.test.ts` that checks that budget is
 * the reason it can't just become a comment.
 */
